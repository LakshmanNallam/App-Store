...
class AppStore extends Component {
  ...
  getfilteredList = currentTab => {
    const {input} = this.state
    const filteredList = appsList.filter(
      eachItem =>
        eachItem.category === currentTab &&
  ...
  getupdatedFilteredList = () => 5

  render() {
    const {currentTab, input} = this.state
    const filteredList = this.getfilteredList(currentTab)

    return (
      <div className="mainDiv">
        <h1>App Store</h1>
        <div className="inpusearchcon">
          <input
            type="search"
            className="inpu"
            onChange={this.updateInpu}
            value={input}
          />
          <img
        ...
        <ul className="ulcon">
          {tabsList.map(eachItem => (
            <TabItem
              eachItem={eachItem}
              fun={this.fun}
              status={eachItem.tabId === currentTab}
              key={eachItem.tabId}
            />
          ))}
        </ul>
        ...
        <div className="jdfdg">
          <ul className="downCon">
            {filteredList.map(eachItem => (
              <AppItem eachItem={eachItem} key={eachItem.appName} />
            ))}
          </ul>
        </div>
      </div>
